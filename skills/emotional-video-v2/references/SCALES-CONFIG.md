# Scales Config — 统一情绪与审美配置

所有 v2 pipeline 的 skill 共享此配置。读取本文档获取当前 scale 值和视觉约束。
...
| Mode | 中文名 | 底色 | 核心特征 | 对应配方 |
|---|---|---|---|---|
| `brutal` | 黑红压迫 | #050505 | acid 绿高亮、网格噪点、背景光晕 | brutal-poster |
| `cinematic` | 电影预告片 | #0a0a0a | 大景深、暗调、金色 accent、电影字幕感 | brutal-poster (cinematic 变体) |
| `editorial` | 杂志封面 | #f5f0eb / #111 | 衬线体、结构化留白、深红 accent | editorial-poster |
| `chaos` | 撕裂拼贴 | #fff / #000 | 反色条、文字旋转、硬阴影、撕裂边缘 | chaos-poster |
| `neon` | 赛博霓虹 | #0a0a1a | neon glow、CRT 扫描线、flicker | neon-poster |
